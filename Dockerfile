...
LABEL description="Multistage Docker image for Substrate Node Template" \
  ...
  image.source="https://github.com/substrate-developer-hub/substrate-meta" \
  image.documentation="https://github.com/substrate-developer-hub/substrate-meta"

# Copy the node binary.
COPY --from=builder /meta/target/release/meta /usr/local/bin

RUN useradd -m -u 1000 -U -s /bin/sh -d /node-dev node-dev && \
  mkdir -p /chain-data /node-dev/.local/share && \
  chown -R node-dev:node-dev /chain-data && \
  ln -s /chain-data /node-dev/.local/share/meta && \
  # unclutter and minimize the attack surface
  rm -rf /usr/bin /usr/sbin && \
  # check if executable works in this container
  /usr/local/bin/meta --version

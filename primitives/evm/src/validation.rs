...
impl<'config, E: From<InvalidEvmTransactionError>> CheckEvmTransaction<'config, E> {
	...
	fn transaction_fee_input(&self) -> Result<(U256, Option<U256>), E> {
		match (
			...
			(None, Some(max_fee_per_gas), None) => Ok((max_fee_per_gas, Some(self.config.base_fee))),
			// EIP-1559 tip.
	...
	pub fn validate_common(&self) -> Result<&Self, E> {
		if self.config.is_transactional {
			...
			let mut gasometer = evm::gasometer::Gasometer::new(self.transaction.gas_limit.unique_saturated_into(), self.config.evm_config);
			let transaction_cost = if self.transaction.to.is_some() {
				evm::gasometer::call_transaction_cost(&self.transaction.input, &self.transaction.access_list)
			} else {
				evm::gasometer::create_transaction_cost(&self.transaction.input, &self.transaction.access_list)
			};
...
mod tests {
	...
	fn default_transaction<'config>(is_transactional: bool) -> CheckEvmTransaction<'config, TestError> {
	...
	fn transaction_none_fee<'config>(is_transactional: bool) -> CheckEvmTransaction<'config, TestError> {
		let mut input = TestCase::default();
	...
	fn transaction_max_fee_low<'config>(is_transactional: bool) -> CheckEvmTransaction<'config, TestError> {
		let mut input = TestCase::default();
	...
	fn transaction_priority_fee_high<'config>(is_transactional: bool) -> CheckEvmTransaction<'config, TestError> {
		let mut input = TestCase::default();
	...
	fn invalid_transaction_mixed_fees<'config>(is_transactional: bool) -> CheckEvmTransaction<'config, TestError> {
		let mut input = TestCase::default();

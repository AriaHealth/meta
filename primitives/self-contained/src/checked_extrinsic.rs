...
use sp_runtime::{
	traits::{self, DispatchInfoOf, Dispatchable, MaybeDisplay, Member, PostDispatchInfoOf, SignedExtension, ValidateUnsigned},
	...
	SelfContainedSignedInfo: Send + Sync + 'static,
{
	...
	fn validate<U: ValidateUnsigned<Call = Self::Call>>(
		...
		match &self.signed {
			CheckedSignature::Signed(id, extra) => Extra::validate(extra, id, &self.function, info, len),
	...
	fn apply<U: ValidateUnsigned<Call = Self::Call>>(
		...
		match self.signed {
			CheckedSignature::Signed(id, extra) => {
				...
				Extra::post_dispatch(Some(pre), info, &post_info, len, &res.map(|_| ()).map_err(|e| e.error))?;
				Ok(res)
			},
			CheckedSignature::Unsigned => {
				...
				Extra::post_dispatch(None, info, &post_info, len, &res.map(|_| ()).map_err(|e| e.error))?;
				Ok(res)
			},

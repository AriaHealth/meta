...
pub mod pallet {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				Signer::<T, T::AuthorityId>::any_account().send_unsigned_transaction(
					|account| Payload { number, block_number, public: account.public.clone(), account_id: account.id.clone() },
			...
			if let Call::do_something_with_signature { signature, payload } = call {
				...
				if !signature_valid {
					...
					ValidTransaction::with_tag_prefix("ExampleOffchainWorker")
						.priority(randomValue.encode()[0].into())
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn do_something_with_signature(
			...
			// Emit an event.
			Self::deposit_event(Event::SomethingStoredSigned { something: payload.number.clone(), account_id: payload.account_id.clone() });
			// Return a successful DispatchResultWithPostInfo
			Ok(())
		}
	}

...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn on_finalize(_n: <T as frame_system::Config>::BlockNumber) {
			...
			let weight_used = Permill::from_rational(weight.total().ref_time(), max_weight.ref_time()).clamp(lower, upper);
			...
			if usage > target {
				...
				<BaseFeePerGas<T>>::mutate(|bf| {
					if let Some(scaled_basefee) = bf.checked_mul(U256::from(coef.deconstruct())) {
						// Normalize to GWEI.
						let increase = scaled_basefee.checked_div(U256::from(1_000_000)).unwrap_or_else(U256::zero);
				...
				<BaseFeePerGas<T>>::mutate(|bf| {
					if let Some(scaled_basefee) = bf.checked_mul(U256::from(coef.deconstruct())) {
						// Normalize to GWEI.
						let decrease = scaled_basefee.checked_div(U256::from(1_000_000)).unwrap_or_else(U256::zero);

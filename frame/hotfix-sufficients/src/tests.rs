...
fn test_hotfix_inc_account_sufficients_returns_error_if_max_addresses_exceeded() {
	new_test_ext().execute_with(|| {
		...
		let addresses = (0..max_address_count + 1 as u64).map(H160::from_low_u64_le).collect::<Vec<H160>>();

		let result = <Pallet<Test>>::hotfix_inc_account_sufficients(RuntimeOrigin::signed(H160::default()), addresses);
...
fn test_hotfix_inc_account_sufficients_increments_if_nonce_nonzero() {
	new_test_ext().execute_with(|| {
		...
		<Pallet<Test>>::hotfix_inc_account_sufficients(RuntimeOrigin::signed(H160::default()), vec![addr_1, addr_2]).unwrap();
...
fn test_hotfix_inc_account_sufficients_increments_with_saturation_if_nonce_nonzero() {
	new_test_ext().execute_with(|| {
		...
		<Pallet<Test>>::hotfix_inc_account_sufficients(RuntimeOrigin::signed(H160::default()), vec![addr]).unwrap();

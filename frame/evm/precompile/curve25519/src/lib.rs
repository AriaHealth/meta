...
impl LinearCostPrecompile for Curve25519Add {
	...
	fn execute(input: &[u8], _: u64) -> Result<(ExitSucceed, Vec<u8>), PrecompileFailure> {
		...
		if input.len() > 320 {
			return Err(PrecompileFailure::Error {
				exit_status: ExitError::Other("input cannot be greater than 320 bytes (10 compressed points)".into()),
			});
		};
...
impl LinearCostPrecompile for Curve25519ScalarMul {
	...
	fn execute(input: &[u8], _: u64) -> Result<(ExitSucceed, Vec<u8>), PrecompileFailure> {
		if input.len() != 64 {
			return Err(PrecompileFailure::Error {
				exit_status: ExitError::Other("input must contain 64 bytes (scalar - 32 bytes, point - 32 bytes)".into()),
			});
		};

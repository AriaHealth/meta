...
fn read_point(input: &[u8], start_inx: usize) -> Result<bn::G1, PrecompileFailure> {
	...
	let px = Fq::from_slice(&input[start_inx..(start_inx + 32)]).map_err(|_| PrecompileFailure::Error {
		exit_status: ExitError::Other("Invalid point x coordinate".into()),
	})?;
	let py = Fq::from_slice(&input[(start_inx + 32)..(start_inx + 64)]).map_err(|_| PrecompileFailure::Error {
	...
	fn execute(handle: &mut impl PrecompileHandle) -> PrecompileResult {
		...
		let ret_val = if handle.input().is_empty() {
			...
			for idx in 0..elements {
				...
				let a_y = Fq::from_slice(&input[idx * 192 + 32..idx * 192 + 64]).map_err(|_| PrecompileFailure::Error {
					exit_status: ExitError::Other("Invalid a argument y coordinate".into()),
				})?;

				let b_a_y = Fq::from_slice(&input[idx * 192 + 64..idx * 192 + 96]).map_err(|_| PrecompileFailure::Error {
					exit_status: ExitError::Other("Invalid b argument imaginary coeff x coordinate".into()),
				})?;

				let b_a_x = Fq::from_slice(&input[idx * 192 + 96..idx * 192 + 128]).map_err(|_| PrecompileFailure::Error {
					exit_status: ExitError::Other("Invalid b argument imaginary coeff y coordinate".into()),
				})?;

				let b_b_y = Fq::from_slice(&input[idx * 192 + 128..idx * 192 + 160]).map_err(|_| PrecompileFailure::Error {
					exit_status: ExitError::Other("Invalid b argument real coeff x coordinate".into()),
				})?;

				let b_b_x = Fq::from_slice(&input[idx * 192 + 160..idx * 192 + 192]).map_err(|_| PrecompileFailure::Error {

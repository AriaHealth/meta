...
use frame_support::{
	...
	traits::{tokens::fungible::Inspect, Currency, ExistenceRequirement, FindAuthor, Get, Imbalance, OnUnbalanced, SignedImbalance, WithdrawReasons},
	weights::Weight,
};
...
pub use fp_evm::{
	Account, CallInfo, CreateInfo, ExecutionInfo, FeeCalculator, InvalidEvmTransactionError, LinearCostPrecompile, Log, Precompile, PrecompileFailure,
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn withdraw(origin: OriginFor<T>, address: H160, value: BalanceOf<T>) -> DispatchResult {
			...
			T::Currency::transfer(&address_account_id, &destination, value, ExistenceRequirement::AllowDeath)?;
		...
		pub fn call(
			...
			Ok(PostDispatchInfo {
				actual_weight: Some(T::GasWeightMapping::gas_to_weight(info.used_gas.unique_saturated_into(), true)),
				pays_fee: Pays::No,
			})
		}
		...
		pub fn create(
			...
			match info {
				...
				CreateInfo {
					...
					Pallet::<T>::deposit_event(Event::<T>::CreatedFailed { address: create_address });
				},
			}
	...
	pub type AccountStorages<T: Config> = StorageDoubleMap<_, Blake2_128Concat, H160, Blake2_128Concat, H256, H256, ValueQuery>;
}
...
impl<T: Config> GasWeightMapping for FixedGasWeightMapping<T> {
	fn gas_to_weight(gas: u64, without_base_weight: bool) -> Weight {
		...
		if without_base_weight {
			weight = weight.saturating_sub(T::BlockWeights::get().get(frame_support::dispatch::DispatchClass::Normal).base_extrinsic);
		}
		weight
	}
	fn weight_to_gas(weight: Weight) -> u64 {
...
where
	...
	C::PositiveImbalance: Imbalance<<C as Currency<<T as frame_system::Config>::AccountId>>::Balance, Opposite = C::NegativeImbalance>,
	C::NegativeImbalance: Imbalance<<C as Currency<<T as frame_system::Config>::AccountId>>::Balance, Opposite = C::PositiveImbalance>,
	...
	U256: UniqueSaturatedInto<<C as Currency<<T as frame_system::Config>::AccountId>>::Balance>,
{
	...
	fn correct_and_deposit_fee(who: &H160, corrected_fee: U256, base_fee: U256, already_withdrawn: Self::LiquidityInfo) -> Self::LiquidityInfo {
		if let Some(paid) = already_withdrawn {
			...
			let refund_imbalance = C::deposit_into_existing(&account_id, refund_amount).unwrap_or_else(|_| C::PositiveImbalance::zero());
			...
			let refund_imbalance = if C::minimum_balance().is_zero() && refund_amount > C::Balance::zero() && C::total_balance(&account_id).is_zero() {
			...
			let adjusted_paid = paid.offset(refund_imbalance).same().unwrap_or_else(|_| C::NegativeImbalance::zero());

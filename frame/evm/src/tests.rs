...
fn ed_0_refund_patch_is_required() {
	new_test_ext().execute_with(|| {
		...
		// Drain funds
		let _ = <<Test as Config>::OnChargeTransaction as OnChargeEVMTransaction<Test>>::withdraw_fee(&evm_addr, U256::from(100)).unwrap();
		...
		assert_eq!(
			<Test as Config>::Currency::deposit_into_existing(&substrate_addr, 5u32.into()).is_err(),
			true
		);
...
fn issuance_after_tip() {
	new_test_ext().execute_with(|| {
		...
		let base_fee: u64 = <Test as Config>::FeeCalculator::min_gas_price().0.unique_saturated_into();
...
fn runner_non_transactional_calls_with_non_balance_accounts_is_ok_without_gas_price() {
	...
	new_test_ext().execute_with(|| {
		let non_balance_account = H160::from_str("7700000000000000000000000000000000000001").unwrap();

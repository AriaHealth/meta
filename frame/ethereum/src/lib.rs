...
use fp_evm::{CallOrCreateInfo, CheckEvmTransaction, CheckEvmTransactionConfig, InvalidEvmTransactionError};
...
use sp_runtime::{
	...
	transaction_validity::{InvalidTransaction, TransactionValidity, TransactionValidityError, ValidTransactionBuilder},
	DispatchErrorWithPostInfo, RuntimeDebug,
};
use sp_std::{marker::PhantomData, prelude::*};

pub use ethereum::{
	AccessListItem, BlockV2 as Block, LegacyTransactionMessage, Log, ReceiptV3 as Receipt, TransactionAction, TransactionV2 as Transaction,
};
...
impl<O: Into<Result<RawOrigin, O>> + From<RawOrigin>> EnsureOrigin<O> for EnsureEthereumTransaction {
	type Success = H160;
	...
	T::RuntimeCall: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
	...
	pub fn check_self_contained(&self) -> Option<Result<H160, TransactionValidityError>> {
		if let Call::transact { transaction } = self {
			let check = || {
				let origin =
					Pallet::<T>::recover_signer(transaction).ok_or(InvalidTransaction::Custom(TransactionValidationError::InvalidSignature as u8))?;
	...
	pub fn validate_self_contained(&self, origin: &H160, dispatch_info: &DispatchInfoOf<T::RuntimeCall>, len: usize) -> Option<TransactionValidity> {
		if let Call::transact { transaction } = self {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_finalize(n: T::BlockNumber) {
			...
			if !to_remove.is_zero() {
				<BlockHash<T>>::remove(U256::from(UniqueSaturatedInto::<u32>::unique_saturated_into(to_remove)));
			}
			Pending::<T>::kill();
		}
		...
		fn on_initialize(_: T::BlockNumber) -> Weight {
			...
			if let Ok(log) = fp_consensus::find_pre_log(&frame_system::Pallet::<T>::digest()) {
				...
				for transaction in block.transactions {
					let source = Self::recover_signer(&transaction).expect("pre-block transaction signature invalid; the block cannot be built");

					Self::validate_transaction_in_block(source, &transaction).expect("pre-block transaction verification failed; the block cannot be built");
					let r = Self::apply_validated_transaction(source, transaction).expect("pre-block apply transaction failed; the block cannot be built");
		...
		fn on_runtime_upgrade() -> Weight {
			frame_support::storage::unhashed::put::<EthereumStorageSchema>(PALLET_ETHEREUM_SCHEMA, &EthereumStorageSchema::V3);
...
impl<T: Config> Pallet<T> {
	...
	fn store_block(post_log: bool, block_number: U256) {
		...
		for (transaction, status, receipt) in Pending::<T>::get() {
			...
			let (logs, used_gas) = match receipt {
				Receipt::Legacy(d) | Receipt::EIP2930(d) | Receipt::EIP1559(d) => (d.logs.clone(), d.used_gas),
			};
		...
		let partial_header = ethereum::PartialHeader {
			...
			timestamp: UniqueSaturatedInto::<u64>::unique_saturated_into(pallet_timestamp::Pallet::<T>::get()),
			extra_data: Vec::new(),
		...
		if post_log {
			let digest = DigestItem::Consensus(FRONTIER_ENGINE_ID, PostLog::Hashes(fp_consensus::Hashes::from_block(block)).encode());
	...
	fn validate_transaction_in_pool(origin: H160, transaction: &Transaction) -> TransactionValidity {
		let transaction_data: TransactionData = transaction.into();
	...
	fn apply_validated_transaction(source: H160, transaction: Transaction) -> DispatchResultWithPostInfo {
		...
		Ok(PostDispatchInfo {
			actual_weight: Some(T::GasWeightMapping::gas_to_weight(used_gas.unique_saturated_into(), true)),
			pays_fee: Pays::No,
		})
	}
	...
	pub fn execute(
		...
		let (input, value, gas_limit, max_fee_per_gas, max_priority_fee_per_gas, nonce, action, access_list) = {
			match transaction {
				...
				Transaction::EIP2930(t) => {
					let access_list: Vec<(H160, Vec<H256>)> = t.access_list.iter().map(|item| (item.address, item.storage_keys.clone())).collect();
					(
						t.input.clone(),
						t.value,
						t.gas_limit,
	...
	pub fn validate_transaction_in_block(origin: H160, transaction: &Transaction) -> Result<(), TransactionValidityError> {
		let transaction_data: TransactionData = transaction.into();
	...
	pub fn migrate_block_v0_to_v2() -> Weight {
		...
		let block_v0 = frame_support::storage::migration::get_storage_value::<ethereum::BlockV0>(Self::name().as_bytes(), item, &[]);
		if let Some(block_v0) = block_v0 {
			weight = weight.saturating_add(db_weights.writes(1));
			let block_v2: ethereum::BlockV2 = block_v0.into();
			frame_support::storage::migration::put_storage_value::<ethereum::BlockV2>(Self::name().as_bytes(), item, &[], block_v2);
		}
		weight
	}
	...
	pub fn pre_migrate_block_v2() -> Result<Vec<u8>, &'static str> {
		...
		let block_v0 = frame_support::storage::migration::get_storage_value::<ethereum::BlockV0>(Self::name().as_bytes(), item, &[]);
		if let Some(block_v0) = block_v0 {
			Ok((block_v0.header.number, block_v0.header.parent_hash, block_v0.transactions.len() as u64).encode())
		} else {
			Ok(Vec::new())
		}
	}
	...
	pub fn post_migrate_block_v2(v0_data: Vec<u8>) -> Result<(), &'static str> {
		let (v0_number, v0_parent_hash, v0_transaction_len): (U256, H256, u64) =
			Decode::decode(&mut v0_data.as_slice()).expect("the state parameter should be something that was generated by pre_upgrade");
		let item = b"CurrentBlock";
		let block_v2 = frame_support::storage::migration::get_storage_value::<ethereum::BlockV2>(Self::name().as_bytes(), item, &[]);
...
impl<T: Config> Get<H256> for IntermediateStateRoot<T> {
	fn get() -> H256 {
		...
		H256::decode(&mut &sp_io::storage::root(version)[..]).expect("Node is configured to use the same hash; qed")
	}
}
	...
	fn from(validation_error: InvalidEvmTransactionError) -> Self {
		match validation_error {
			InvalidEvmTransactionError::GasLimitTooLow => {
				InvalidTransactionWrapper(InvalidTransaction::Custom(TransactionValidationError::GasLimitTooLow as u8))
			},
			InvalidEvmTransactionError::GasLimitTooHigh => {
				InvalidTransactionWrapper(InvalidTransaction::Custom(TransactionValidationError::GasLimitTooHigh as u8))
			},
			...
			InvalidEvmTransactionError::PriorityFeeTooHigh => {
				InvalidTransactionWrapper(InvalidTransaction::Custom(TransactionValidationError::MaxFeePerGasTooLow as u8))
			},
			...
			InvalidEvmTransactionError::InvalidChainId => {
				InvalidTransactionWrapper(InvalidTransaction::Custom(TransactionValidationError::InvalidChainId as u8))
			},
		}
	}
}

...
fn transaction_with_max_extrinsic_gas_limit_should_success_pre_dispatch() {
	...
	let limits: frame_system::limits::BlockWeights = <Test as frame_system::Config>::BlockWeights::get();
	let max_extrinsic = limits.get(DispatchClass::Normal).max_extrinsic.unwrap();
	let max_extrinsic_gas = <Test as pallet_evm::Config>::GasWeightMapping::weight_to_gas(max_extrinsic);
	...
	ext.execute_with(|| {
		...
		assert_ok!(call.pre_dispatch_self_contained(&source, &extrinsic.get_dispatch_info(), 0).unwrap());
	});
}
...
fn transaction_with_gas_limit_greater_than_max_extrinsic_should_fail_pre_dispatch() {
	...
	let max_extrinsic_gas = <Test as pallet_evm::Config>::GasWeightMapping::weight_to_gas(max_extrinsic + base_extrinsic);
	...
	ext.execute_with(|| {
		...
		assert_err!(
			call.pre_dispatch_self_contained(&source, &extrinsic.get_dispatch_info(), 0).unwrap(),
...
fn transaction_with_to_low_nonce_should_not_work() {
	...
	ext.execute_with(|| {
		...
		assert_err!(
			call2.validate_self_contained(&source2, &extrinsic2.get_dispatch_info(), 0).unwrap(),
...
fn transaction_with_invalid_chain_id_should_fail_in_block() {
	...
	ext.execute_with(|| {
		let transaction = eip1559_erc20_creation_unsigned_transaction().sign(&alice.private_key, Some(1));
		...
		assert_err!(
			extrinsic.apply::<Test>(&dispatch_info, 0),
			TransactionValidityError::Invalid(InvalidTransaction::Custom(fp_ethereum::TransactionValidationError::InvalidChainId as u8,))
		);
	});
}
...
fn self_contained_transaction_with_extra_gas_should_adjust_weight_with_post_dispatch() {
	...
	ext.execute_with(|| {
		...
		let dispatch_info = extrinsic.get_dispatch_info();
		let post_dispatch_weight = extrinsic.apply::<Test>(&dispatch_info, 0).unwrap().unwrap().actual_weight.unwrap();

		let expected_weight = base_extrinsic_weight.saturating_add(post_dispatch_weight);
		let actual_weight = *frame_system::Pallet::<Test>::block_weight().get(DispatchClass::Normal);
		assert_eq!(
			expected_weight,
			actual_weight,

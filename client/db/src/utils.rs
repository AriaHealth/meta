...
pub fn open_database<Block: BlockT, C>(client: Arc<C>, config: &DatabaseSettings) -> Result<Arc<dyn Database<DbHash>>, String>
where
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{
	let db: Arc<dyn Database<DbHash>> = match &config.source {
		...
		DatabaseSource::RocksDb { path, .. } => open_kvdb_rocksdb::<Block, C>(client, path, true, &config.source)?,
		DatabaseSource::Auto {
			paritydb_path, rocksdb_path, ..
...
fn open_parity_db<Block: BlockT, C>(client: Arc<C>, path: &Path, _source: &DatabaseSource) -> Result<Arc<dyn Database<DbHash>>, String>
where
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{
	// first upgrade database to required version
...
fn open_parity_db<Block: BlockT, C>(_client: Arc<C>, _path: &Path, _source: &DatabaseSource) -> Result<Arc<dyn Database<DbHash>>, String>
where
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{

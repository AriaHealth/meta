...
impl<Block: BlockT> Backend<Block> {
	pub fn open<C: sp_blockchain::HeaderBackend<Block>>(client: Arc<C>, database: &DatabaseSource, db_config_dir: &Path) -> Result<Self, String> {
		Self::new(
			client,
			&DatabaseSettings {
	...
	pub fn new<C: sp_blockchain::HeaderBackend<Block>>(client: Arc<C>, config: &DatabaseSettings) -> Result<Self, String> {
...
impl<Block: BlockT> MetaDb<Block> {
	...
	pub fn write_ethereum_schema(&self, new_cache: Vec<(EthereumStorageSchema, H256)>) -> Result<(), String> {
		let mut transaction = sp_database::Transaction::new();

		transaction.set(crate::columns::META, &PALLET_ETHEREUM_SCHEMA_CACHE.encode(), &new_cache.encode());
...
impl<Block: BlockT> MappingDb<Block> {
	...
	pub fn transaction_metadata(&self, ethereum_transaction_hash: &H256) -> Result<Vec<TransactionMetadata<Block>>, String> {

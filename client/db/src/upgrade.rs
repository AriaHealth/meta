...
pub(crate) fn upgrade_db<Block: BlockT, C>(client: Arc<C>, db_path: &Path, source: &DatabaseSource) -> UpgradeResult<()>
where
...
pub(crate) fn migrate_1_to_2_rocks_db<Block: BlockT, C>(client: Arc<C>, db_path: &Path) -> UpgradeResult<UpgradeVersion1To2Summary>
where
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{
...
pub(crate) fn migrate_1_to_2_parity_db<Block: BlockT, C>(client: Arc<C>, db_path: &Path) -> UpgradeResult<UpgradeVersion1To2Summary>
where
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{
	log::info!("🔨 Running Frontier DB migration from version 1 to version 2. Please wait.");
	...
	let db = parity_db::Db::open_or_create(&db_cfg).map_err(|_| io::Error::new(ErrorKind::Other, "Failed to open db"))?;
...
mod tests {
	...
	use substrate_test_runtime_client::{prelude::*, DefaultTestClientBuilderExt, TestClientBuilder};
	...
	type OpaqueBlock = Block<Header<u64, BlakeTwo256>, substrate_test_runtime_client::runtime::Extrinsic>;

	pub fn open_frontier_backend<C>(client: Arc<C>, setting: &crate::DatabaseSettings) -> Result<Arc<crate::Backend<OpaqueBlock>>, String>
	where
	...
	fn upgrade_1_to_2_works() {
		...
		for setting in settings {
			let (client, _) = TestClientBuilder::new().build_with_native_executor::<substrate_test_runtime_client::runtime::RuntimeApi, _>(None);
			...
			let mut transaction_hashes = vec![];
			{
				...
				let backend = open_frontier_backend(client.clone(), &setting).expect("a temporary db was created");
				...
				for _ in 0..1000 {
					...
					let mut builder = client
						.new_block_at(&BlockId::Hash(previous_canon_block_hash), Default::default(), false)
						.unwrap();
					builder.push_storage_change(vec![1], None).unwrap();
					let block = builder.build().unwrap().block;
					...
					transaction.set(crate::columns::BLOCK_MAPPING, &ethhash.encode(), &orphan_block_hash.encode());
					...
					transaction.set(crate::columns::TRANSACTION_MAPPING, &eth_tx_hash.encode(), &metadata.encode());
			...
			let mut version_file = std::fs::File::create(version_path).expect("db version file path created");
			...
			let backend = open_frontier_backend(client, &setting).expect("a temporary db was created");
			for (i, original_ethereum_hash) in ethereum_hashes.iter().enumerate() {
				...
				let mapped_block = backend.mapping().block_hash(original_ethereum_hash).unwrap().unwrap();
				...
				let mapped_transaction = backend
					...
					.unwrap();
				assert!(mapped_transaction.into_iter().any(|tx| tx.block_hash == *canon_substrate_block_hash));
			}
	...
	fn create_db_with_current_version_works() {
		...
		let mut file = std::fs::File::open(crate::upgrade::version_file_path(&path)).expect("file exist");

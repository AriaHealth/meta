		...
		S: Serializer,
	{
		match *self {
			BlockNumber::Hash { hash, require_canonical } => {
				serializer.serialize_str(&format!("{{ 'hash': '{}', 'requireCanonical': '{}'  }}", hash, require_canonical))
			},
...
impl<'a> Visitor<'a> for BlockNumberVisitor {
	...
	fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
		write!(formatter, "a block number or 'latest', 'safe', 'finalized', 'earliest' or 'pending'")
	}
				...
				Some(key) => match key.as_str() {
					"blockNumber" => {
						...
						if let Some(stripped) = value.strip_prefix("0x") {
							let number = u64::from_str_radix(stripped, 16).map_err(|e| Error::custom(format!("Invalid block number: {}", e)))?;
		...
		E: Error,
	{
		match value {
			...
			_ => value
				.parse::<u64>()
				.map(BlockNumber::Num)
				.map_err(|_| Error::custom("Invalid block number: non-decimal or missing 0x prefix".to_string())),
		}
	}

	...
	C: sp_blockchain::HeaderBackend<B>,
{
	...
	pub fn query(&self, column: &Column, key: &MappingKey, value: &Option<MappingValue<B::Hash>>) -> sc_cli::Result<()> {
		match self.cmd.operation {
			Operation::Create => match (key, value) {
				...
				(MappingKey::EthBlockOrTransactionHash(ethereum_block_hash), Some(MappingValue::SubstrateBlockHash(substrate_block_hash))) => {
			...
			Operation::Read => match (column, key) {
				...
				(Column::Transaction, MappingKey::EthBlockOrTransactionHash(ethereum_transaction_hash)) => {
					let value = self.backend.mapping().transaction_metadata(ethereum_transaction_hash)?;
					println!("{:?}", value);
				},
			...
			Operation::Delete => return Err("Delete operation is not supported for non-static keys".to_string().into()),
		}
		Ok(())
	}
}

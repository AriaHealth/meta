...
pub fn maybe_deserialize_value<B: BlockT>(operation: &Operation, value: Option<&PathBuf>) -> sc_cli::Result<Option<DbValue<B::Hash>>> {
...
pub trait FrontierDbMessage {
	fn key_value_error<K: core::fmt::Debug, V: core::fmt::Debug>(&self, key: K, value: &V) -> sc_cli::Error {
		format!("Key `{:?}` and Value `{:?}` are not compatible with this operation", key, value).into()
	}

	fn key_column_error<K: core::fmt::Debug, V: core::fmt::Debug>(&self, key: K, value: &V) -> sc_cli::Error {
		format!("Key `{:?}` and Column `{:?}` are not compatible with this operation", key, value).into()
	}

...
impl FrontierDbCmd {
	pub fn run<C, B: BlockT>(&self, client: Arc<C>, backend: Arc<fc_db::Backend<B>>) -> sc_cli::Result<()>
	where
		...
		C: sp_blockchain::HeaderBackend<B>,
	{
		match self.column {
			Column::Meta => {
				...
				let value = match utils::maybe_deserialize_value::<B>(&self.operation, self.value.as_ref())? {
			...
			Column::Block | Column::Transaction => {
				...
				let key = MappingKey::EthBlockOrTransactionHash(H256::from_str(&self.key).expect("H256 provided key"));

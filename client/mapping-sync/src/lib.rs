...
pub fn sync_block<Block: BlockT>(backend: &fc_db::Backend<Block>, header: &Block::Header) -> Result<(), String> {
...
pub fn sync_genesis_block<Block: BlockT, C>(client: &C, backend: &fc_db::Backend<Block>, header: &Block::Header) -> Result<(), String>
where
	...
	C::Api: EthereumRuntimeRPCApi<Block>,
{
		...
		.map_err(|e| format!("{:?}", e))?
	{
		let block = if api_version > 1 {
			...
			#[allow(deprecated)]
			let legacy_block = client.runtime_api().current_block_before_version_2(&id).map_err(|e| format!("{:?}", e))?;
			legacy_block.map(|block| block.into())
		};
		let block_hash = block.ok_or_else(|| "Ethereum genesis block not found".to_string())?.header.hash();
	...
	B: sp_blockchain::HeaderBackend<Block> + sp_blockchain::Backend<Block>,
{
	...
	while let Some(checking_tip) = current_syncing_tips.pop() {
		if let Some(checking_header) = fetch_header(substrate_backend, frontier_backend, checking_tip, sync_from)? {
	...
	B: sp_blockchain::HeaderBackend<Block> + sp_blockchain::Backend<Block>,
{
	...
	for _ in 0..limit {
		synced_any = synced_any || sync_one_block(client, substrate_backend, frontier_backend, sync_from, strategy)?;
	}

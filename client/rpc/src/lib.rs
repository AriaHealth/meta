...
pub use self::{
	...
	net::Net,
	overrides::{OverrideHandle, RuntimeApiStorageOverride, SchemaV1Override, SchemaV2Override, SchemaV3Override, StorageOverride},
...
pub mod frontier_backend_client {
	...
	pub fn native_block_id<B: BlockT, C>(client: &C, backend: &fc_db::Backend<B>, number: Option<BlockNumber>) -> RpcResult<Option<BlockId<B>>>
	where
	...
	pub fn load_hash<B: BlockT, C>(client: &C, backend: &fc_db::Backend<B>, hash: H256) -> RpcResult<Option<BlockId<B>>>
	where
		B: BlockT<Hash = H256> + Send + Sync + 'static,
		C: HeaderBackend<B> + Send + Sync + 'static,
	{
	...
	pub fn load_cached_schema<B: BlockT, C>(backend: &fc_db::Backend<B>) -> RpcResult<Option<Vec<(EthereumStorageSchema, H256)>>>
	where
		B: BlockT<Hash = H256> + Send + Sync + 'static,
		C: HeaderBackend<B> + Send + Sync + 'static,
	{
	...
	pub fn write_cached_schema<B: BlockT, C>(backend: &fc_db::Backend<B>, new_cache: Vec<(EthereumStorageSchema, H256)>) -> RpcResult<()>
	where
		B: BlockT<Hash = H256> + Send + Sync + 'static,
		C: HeaderBackend<B> + Send + Sync + 'static,
	{
...
pub fn err<T: ToString>(code: i32, message: T, data: Option<&[u8]>) -> jsonrpsee::core::Error {
	jsonrpsee::core::Error::Call(jsonrpsee::types::error::CallError::Custom(jsonrpsee::types::error::ErrorObject::owned(
		code,
		message.to_string(),
		data.map(|bytes| jsonrpsee::core::to_json_raw_value(&format!("0x{}", hex::encode(bytes))).expect("fail to serialize data")),
	)))
}
...
mod tests {
	...
	fn open_frontier_backend<C>(client: Arc<C>, path: PathBuf) -> Result<Arc<fc_db::Backend<OpaqueBlock>>, String>
	where
	...
	fn substrate_block_hash_one_to_many_works() {
		...
		// A1 -> B1
		let mut builder = client.new_block_at(&BlockId::Hash(a1_hash), Default::default(), false).unwrap();
		builder.push_storage_change(vec![1], None).unwrap();
		...
		assert_eq!(
			super::frontier_backend_client::load_hash(client.as_ref(), frontier_backend.as_ref(), ethereum_block_hash)
				.unwrap()
				.unwrap(),
		...
		let mut builder = client.new_block_at(&BlockId::Hash(b2_hash), Default::default(), false).unwrap();
		builder.push_storage_change(vec![1], None).unwrap();

			...
			if &key_address == address {
				match message {
					TransactionMessage::Legacy(m) => {
						let signing_message = libsecp256k1::Message::parse_slice(&m.hash()[..]).map_err(|_| internal_err("invalid signing message"))?;
						...
						transaction = Some(EthereumTransaction::Legacy(ethereum::LegacyTransaction {
							...
							value: m.value,
							input: m.input,
							signature: ethereum::TransactionSignature::new(v, r, s).ok_or_else(|| internal_err("signer generated invalid signature"))?,
						}));
					},

	...
	C: HeaderBackend<B> + Send + Sync + 'static,
{
	pub fn new(
		...
		let starting_block = UniqueSaturatedInto::<u64>::unique_saturated_into(client.info().best_number);
		Self {
			pool,
			client,
			network,
...
impl EthSubscriptionResult {
	...
	pub fn logs(block: EthereumBlock, receipts: Vec<ethereum::ReceiptV3>, params: &FilteredParams) -> Vec<Log> {
		let block_hash = Some(H256::from(keccak_256(&rlp::encode(&block.header))));
		let mut logs: Vec<Log> = vec![];
		let mut log_index: u32 = 0;
		for (receipt_index, receipt) in receipts.into_iter().enumerate() {
			let receipt_logs = match receipt {
				ethereum::ReceiptV3::Legacy(d) | ethereum::ReceiptV3::EIP2930(d) | ethereum::ReceiptV3::EIP1559(d) => d.logs,
			};
	...
	fn add_log(block_hash: H256, ethereum_log: &ethereum::Log, block: &EthereumBlock, params: &FilteredParams) -> bool {
		let log = Log {
		...
		if params.filter.is_some() {
			let block_number = UniqueSaturatedInto::<u64>::unique_saturated_into(block.header.number);
	...
	BE::State: StateBackend<BlakeTwo256>,
{
	fn subscribe(&self, mut sink: SubscriptionSink, kind: Kind, params: Option<Params>) -> SubscriptionResult {
			...
			match kind {
				Kind::Logs => {
					let stream = client
						...
						.filter_map(move |notification| {
							if notification.is_new_best {
								...
								match (receipts, block) {
									(Some(receipts), Some(block)) => futures::future::ready(Some((block, receipts))),
						...
						.flat_map(move |(block, receipts)| futures::stream::iter(EthSubscriptionResult::logs(block, receipts, &filtered_params)))
				...
				Kind::Syncing => {
					...
					async fn status<C: HeaderBackend<B>, B: BlockT, H: ExHashT + Send + Sync>(
						...
						if network.is_major_syncing() {
							...
							// Best imported block.
							let current_block = UniqueSaturatedInto::<u64>::unique_saturated_into(client.info().best_number);
					...
					while (stream.next().await).is_some() {
						...
						if syncing_status != last_syncing_status {
							let _ = sink.send(&PubSubResult::SyncState(status(client.clone(), network.clone(), starting_block).await));
						}
		...
		.boxed();
		self.subscriptions.spawn("frontier-rpc-subscription", Some("rpc"), fut.map(drop).boxed());
		Ok(())
	}
}

	...
	C::Api: EthereumRuntimeRPCApi<Block>,
{
	...
	fn current_block(&self, block: &BlockId<Block>) -> Option<ethereum::BlockV2> {
		...
		let api_version = if let Ok(Some(api_version)) = api.api_version::<dyn EthereumRuntimeRPCApi<Block>>(block) {
			api_version
		} else {
			return None;
		};
	...
	fn is_eip1559(&self, block: &BlockId<Block>) -> bool {
		if let Ok(Some(api_version)) = self.client.runtime_api().api_version::<dyn EthereumRuntimeRPCApi<Block>>(block) {

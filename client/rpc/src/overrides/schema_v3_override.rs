	...
	BE::State: StateBackend<BlakeTwo256>,
{
	...
	fn current_block(&self, block: &BlockId<B>) -> Option<ethereum::BlockV2> {
		self.query_storage::<ethereum::BlockV2>(block, &StorageKey(storage_prefix_build(PALLET_ETHEREUM, ETHEREUM_CURRENT_BLOCK)))
	}

	/// Return the current receipt.
	fn current_receipts(&self, block: &BlockId<B>) -> Option<Vec<ethereum::ReceiptV3>> {
		self.query_storage::<Vec<ethereum::ReceiptV3>>(block, &StorageKey(storage_prefix_build(PALLET_ETHEREUM, ETHEREUM_CURRENT_RECEIPTS)))
	}
	...
	fn elasticity(&self, block: &BlockId<B>) -> Option<Permill> {
		...
		let elasticity = self.query_storage::<Permill>(block, &StorageKey(storage_prefix_build(PALLET_BASE_FEE, BASE_FEE_ELASTICITY)));

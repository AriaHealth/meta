...
impl<K: Eq + core::hash::Hash, V: Encode> LRUCacheByteLimited<K, V> {
	pub fn new(cache_name: &'static str, max_size: u64, prometheus_registry: Option<prometheus_endpoint::Registry>) -> Self {
	...
	pub(crate) fn register(
		...
		Ok(Self {
			hits: prometheus_endpoint::register(
				prometheus::IntCounter::new(format!("frontier_eth_{}_hits", cache_name), format!("Hits of eth {} cache.", cache_name))?,
				registry,
			)?,

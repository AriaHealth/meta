	...
	A: ChainApi<Block = B> + 'static,
{
	pub async fn transaction_by_hash(&self, hash: H256) -> Result<Option<Transaction>> {
		...
		let (hash, index) = match frontier_backend_client::load_transactions::<B, C>(client.as_ref(), backend.as_ref(), hash, true)
			.map_err(|err| internal_err(format!("{:?}", err)))?
		{
			...
			None => {
				...
				let api_version = if let Ok(Some(api_version)) = api.api_version::<dyn EthereumRuntimeRPCApi<B>>(&best_block) {
					api_version
				} else {
		...
		let id =
			match frontier_backend_client::load_hash::<B, C>(client.as_ref(), backend.as_ref(), hash).map_err(|err| internal_err(format!("{:?}", err)))? {
				Some(hash) => hash,
				_ => return Ok(None),
			};
		...
		let schema = frontier_backend_client::onchain_storage_schema::<B, C, BE>(client.as_ref(), id);
	...
	pub async fn transaction_by_block_hash_and_index(&self, hash: H256, index: Index) -> Result<Option<Transaction>> {
		let client = Arc::clone(&self.client);
		let block_data_cache = Arc::clone(&self.block_data_cache);
		let backend = Arc::clone(&self.backend);
		...
		match (block, statuses) {
			(Some(block), Some(statuses)) => {
				if let (Some(transaction), Some(status)) = (block.transactions.get(index), statuses.get(index)) {
	...
	pub async fn transaction_by_block_number_and_index(&self, number: BlockNumber, index: Index) -> Result<Option<Transaction>> {
		let client = Arc::clone(&self.client);
		let block_data_cache = Arc::clone(&self.block_data_cache);
		let backend = Arc::clone(&self.backend);

		let id = match frontier_backend_client::native_block_id::<B, C>(client.as_ref(), backend.as_ref(), Some(number))? {
			Some(id) => id,
			None => return Ok(None),
		};
		let substrate_hash = client
						...
						ethereum::ReceiptV3::Legacy(ref d) => {
							...
							let cumulative_gas: u32 = receipts[..index]
								...
								.map(|r| match r {
									...
									_ => Err(internal_err(format!("Unknown receipt for request {}", hash))),
								})
								.sum::<Result<u32>>()?;
							(d.logs.clone(), d.logs_bloom, d.status_code, U256::from(cumulative_gas), d.used_gas)
						},
						_ => return Err(internal_err(format!("Unknown receipt for request {}", hash))),
					}
				} else {
					match receipt {
						ethereum::ReceiptV3::Legacy(ref d) | ethereum::ReceiptV3::EIP2930(ref d) | ethereum::ReceiptV3::EIP1559(ref d) => {
							...
							let gas_used = if index > 0 {
								...
								let previous_gas_used = match previous_receipt {
									ethereum::ReceiptV3::Legacy(d) | ethereum::ReceiptV3::EIP2930(d) | ethereum::ReceiptV3::EIP1559(d) => d.used_gas,
								};
						...
						if cumulative_receipts.len() > 0 {
							...
							pre_receipts_log_index = Some(
								cumulative_receipts
									.iter()
									.map(|r| match r {
										ethereum::ReceiptV3::Legacy(d) | ethereum::ReceiptV3::EIP2930(d) | ethereum::ReceiptV3::EIP1559(d) => d.logs.len() as u32,
									})
									.sum::<u32>(),
							);
						}

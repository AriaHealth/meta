	...
	A: ChainApi<Block = B> + 'static,
{
	pub fn balance(&self, address: H160, number: Option<BlockNumber>) -> Result<U256> {
		...
		} else if let Ok(Some(id)) = frontier_backend_client::native_block_id::<B, C>(self.client.as_ref(), self.backend.as_ref(), Some(number)) {
			Ok(
				self
					.client
					.runtime_api()
	...
	pub fn storage_at(&self, address: H160, index: U256, number: Option<BlockNumber>) -> Result<H256> {
		let number = number.unwrap_or(BlockNumber::Latest);
		if number == BlockNumber::Pending {
			let api = pending_runtime_api(self.client.as_ref(), self.graph.as_ref())?;
			Ok(

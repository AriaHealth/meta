	...
	EGA: EstimateGasAdapter,
{
	pub fn call(&self, request: CallRequest, number: Option<BlockNumber>) -> Result<Bytes> {
		...
		let (id, api) = match frontier_backend_client::native_block_id::<B, C>(self.client.as_ref(), self.backend.as_ref(), number)? {
		...
		let api_version = if let Ok(Some(api_version)) = api.api_version::<dyn EthereumRuntimeRPCApi<B>>(&id) {
			api_version
		} else {
				...
				if api_version == 1 {
					...
					let info = api
						.call(
							...
							nonce,
							false,
							Some(access_list.into_iter().map(|item| (item.address, item.storage_keys)).collect()),
						)
						.map_err(|err| internal_err(format!("runtime error: {:?}", err)))?
						.map_err(|err| internal_err(format!("execution fatal: {:?}", err)))?;
	...
	pub async fn estimate_gas(&self, request: CallRequest, _: Option<BlockNumber>) -> Result<U256> {
		...
		let api_version = if let Ok(Some(api_version)) = client
			.runtime_api()
		...
		let ExecutableResult { data, exit_reason, used_gas } = executable(request.clone(), highest, api_version, client.runtime_api(), estimate_mode)?;
		match exit_reason {
			ExitReason::Succeed(_) => (),
			ExitReason::Error(ExitError::OutOfGas) => return Err(internal_err(format!("gas required exceeds allowance {}", cap))),
			...
			ExitReason::Revert(revert) => {
				if request.gas.is_some() || request.gas_price.is_some() {
					...
					let ExecutableResult {
						...
						exit_reason,
						used_gas: _,
					} = executable(request.clone(), max_gas_limit, api_version, client.runtime_api(), estimate_mode)?;
					match exit_reason {
						ExitReason::Succeed(_) => return Err(internal_err(format!("gas required exceeds allowance {}", cap))),
		...
		#[cfg(feature = "rpc_binary_search_estimate")]
		{
			...
			while (highest - lowest) > U256::one() {
				...
				match exit_reason {
					...
					ExitReason::Revert(_) | ExitReason::Error(ExitError::OutOfGas) | ExitReason::Error(ExitError::InvalidCode(_)) => {
						lowest = mid;
					},
					other => error_on_execution_failure(&other, &data)?,
				}
...
pub fn error_on_execution_failure(reason: &ExitReason, data: &[u8]) -> Result<()> {
	match reason {
		...
		ExitReason::Fatal(e) => Err(crate::internal_err_with_data(format!("evm fatal: {:?}", e), &[])),
	}
}

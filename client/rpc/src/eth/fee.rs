	...
	BE::State: StateBackend<BlakeTwo256>,
{
	...
	pub fn fee_history(&self, block_count: U256, newest_block: BlockNumber, reward_percentiles: Option<Vec<f64>>) -> Result<FeeHistory> {
		...
		if let Ok(Some(id)) = frontier_backend_client::native_block_id::<B, C>(self.client.as_ref(), self.backend.as_ref(), Some(newest_block)) {
			...
			// Tip of the chain.
			let best_number = UniqueSaturatedInto::<u64>::unique_saturated_into(self.client.info().best_number);
			...
			if let Ok(fee_history_cache) = &self.fee_history_cache.lock() {
				...
				if let (Some(last_gas_used), Some(last_fee_per_gas)) = (response.gas_used_ratio.last(), response.base_fee_per_gas.last()) {
					...
					let elasticity = handler.elasticity(&id).unwrap_or(default_elasticity).deconstruct();
	...
	pub fn max_priority_fee_per_gas(&self) -> Result<U256> {
		...
		let highest = UniqueSaturatedInto::<u64>::unique_saturated_into(self.client.info().best_number);

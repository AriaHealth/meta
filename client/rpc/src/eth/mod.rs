	...
	A: ChainApi<Block = B> + 'static,
{
	...
	fn uncle_by_block_number_and_index(&self, number: BlockNumber, index: Index) -> Result<Option<RichBlock>> {
	...
	async fn transaction_by_block_hash_and_index(&self, hash: H256, index: Index) -> Result<Option<Transaction>> {
		self.transaction_by_block_hash_and_index(hash, index).await
	}

	async fn transaction_by_block_number_and_index(&self, number: BlockNumber, index: Index) -> Result<Option<Transaction>> {
	...
	async fn estimate_gas(&self, request: CallRequest, number: Option<BlockNumber>) -> Result<U256> {
	...
	fn fee_history(&self, block_count: U256, newest_block: BlockNumber, reward_percentiles: Option<Vec<f64>>) -> Result<FeeHistory> {
...
fn rich_block_build(
	...
	Rich {
		inner: Block {
			header: Header {
				hash: Some(hash.unwrap_or_else(|| H256::from(keccak_256(&rlp::encode(&block.header))))),
			...
			transactions: {
				if full_transactions {
					...
					BlockTransactions::Hashes(block.transactions.iter().map(|transaction| transaction.hash()).collect())
				}
			},
...
fn transaction_build(
	...
	// Block hash.
	transaction.block_hash = block.as_ref().map(|block| H256::from(keccak_256(&rlp::encode(&block.header))));
	// Block number.
	transaction.block_number = block.as_ref().map(|block| block.header.number);
	// Transaction index.
	transaction.transaction_index = status
		.as_ref()
		.map(|status| U256::from(UniqueSaturatedInto::<u32>::unique_saturated_into(status.transaction_index)));
	// From.
...
fn pending_runtime_api<'a, B: BlockT, C, BE, A: ChainApi>(client: &'a C, graph: &'a Pool<A>) -> Result<sp_api::ApiRef<'a, C::Api>>
where
	B: BlockT<Hash = H256> + Send + Sync + 'static,
	C: ProvideRuntimeApi<B> + StorageProvider<B, BE>,
	C: HeaderBackend<B> + Send + Sync + 'static,
	...
	A: ChainApi<Block = B> + 'static,
{
	...
	if let Ok(Some(header)) = client.header(best) {
		...
		api
			.initialize_block(&parent_hash, &header)

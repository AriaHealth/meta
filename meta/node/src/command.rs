...
pub fn run() -> sc_cli::Result<()> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			use meta_runtime::{Block, ExistentialDeposit};

			let runner = cli.create_runner(cmd)?;
			match cmd {
				BenchmarkCmd::Pallet(cmd) => runner.sync_run(|config| cmd.run::<Block, service::ExecutorDispatch>(config)),
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move { service::new_full(config, &cli).map_err(sc_cli::Error::Service) })
		},
	}
}
